...
import { motion, Variants } from "framer-motion"; // <-- Tambahan Variants di sini
...
export default function ProfileForm({ initialData }: ProfileFormProps) {
  ...
  // <-- Penambahan tipe data eksplisit: Variants -->
    ...
    <div className="relative w-full max-w-4xl mx-auto flex justify-center transform scale-[1.15] md:scale-[1.20] origin-top mb-24 mt-4">
      ...
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden md:overflow-visible flex justify-center">
        ...
        <motion.div 
          ...
          className="absolute top-4 -left-8 md:-left-28 text-[100px] md:text-[140px] opacity-[0.15] filter blur-[2px]"
        >
          🧴
        </motion.div>
        
        <motion.div 
          animate={{ scale: [1, 1.3, 1], opacity: [0.1, 0.3, 0.1], rotate: [0, 90, 180] }} 
          transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }} 
          className="absolute top-12 right-0 md:-right-10 text-[70px] md:text-[90px] filter blur-[1px] text-yellow-500"
        >
          ✨
        </motion.div>
        ...
        <motion.div 
          ...
          className="absolute top-64 -right-10 md:-right-24 text-[110px] md:text-[150px] opacity-[0.15] filter blur-[2px]"
        >
          🧪
        </motion.div>
        ...
        <motion.div 
          ...
          className="absolute top-80 -left-6 md:-left-16 text-[80px] md:text-[110px] opacity-20 filter blur-[1px]"
        >
          🫧
        </motion.div>
        ...
        <motion.div 
          ...
          className="absolute bottom-40 -left-10 md:-left-20 text-[100px] md:text-[130px] opacity-[0.15] filter blur-[2px]"
        >
          🌿
        </motion.div>
        ...
        <motion.div 
          ...
          className="absolute bottom-16 -right-8 md:-right-16 text-[90px] md:text-[120px] opacity-20 filter blur-[2px]"
        >
          💧
        </motion.div>
        ...
        <motion.div 
          ...
          className="absolute -bottom-20 right-1/4 text-[130px] md:text-[180px] opacity-10 filter blur-[3px]"
        >
          🌸
        </motion.div>
      ...
      >
        ...
        <motion.div variants={itemVariants} className="space-y-3">
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 font-medium focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent transition-all text-sm bg-gray-50/50 hover:bg-gray-50 shadow-inner"
          />
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {["Normal", "Kering", "Berminyak", "Kombinasi"].map((type) => (
              <button
                key={type}
                type="button"
                onClick={() => handleBaseSkinChange(type)}
                className={`py-3 px-4 rounded-xl border-2 text-sm font-bold transform active:scale-95 transition-all ${
                  baseSkin === type
          ...
          <div className="flex items-center gap-3 p-4 bg-red-50/50 border border-red-100 rounded-xl mt-3 transition-colors hover:bg-red-50">
            ...
            />
            <label htmlFor="sensitive" className="text-sm font-bold text-red-900 cursor-pointer select-none">
              Kulit saya rentan Kemerahan / Sensitif 🌡️
            </label>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="flex items-center gap-3 p-4 bg-pink-50/50 border border-pink-100 rounded-xl transition-colors hover:bg-pink-50">
          ...
          />
          <label htmlFor="pregnant" className="text-sm font-bold text-pink-900 cursor-pointer select-none">
        ...
        <motion.div variants={itemVariants} className="pt-4">
          <button
            ...
            className="w-full py-4 bg-black text-white font-bold rounded-xl hover:bg-gray-800 transition-all transform active:scale-95 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed shadow-[0_8px_30px_rgb(0,0,0,0.12)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.2)] text-lg flex justify-center items-center"
          >
            {isLoading ? (
