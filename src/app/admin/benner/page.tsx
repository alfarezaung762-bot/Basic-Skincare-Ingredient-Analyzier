    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 md:p-8 lg:p-12 relative overflow-hidden">
      <div className="max-w-7xl mx-auto space-y-8 relative z-10">
        ...
        <AdminHeader 
          ...
          subtitle="Kelola gambar banner dinamis yang muncul di halaman beranda."
        />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-24 bg-slate-50 dark:bg-slate-950/50 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
              <span className="text-4xl block mb-3 opacity-50">🖼️</span>
              <p className="text-slate-500 dark:text-slate-400 font-medium">Belum ada banner. Ayo tambahkan banner pertamamu!</p>
            </div>
          ) : (
              ...
              <AnimatePresence>
                {banners.map((banner) => (
                  <motion.div 
                    ...
                    key={banner.id} 
                    className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col group relative"
                  >
                    ...
                    <div className="p-5 flex-1 flex flex-col justify-between gap-4">
                      <div>
                        <p className="text-sm text-slate-500 dark:text-slate-400 truncate">{banner.altText || "Tanpa alt text"}</p>
