...
export default function EditIngredientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [skinTypes, setSkinTypes] = useState({
    Normal: false, Kering: false, Berminyak: false, Kombinasi: false,
  });
    ...
    const fetchOldData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setFormData({
            name: data.name, aliases: data.aliases || "", type: data.type, 
            functionalCategory: data.functionalCategory || "UMUM", // Tarik data kategori fungsional lama
            benefits: data.benefits, warnings: data.warnings || "",
            comedogenicRating: data.comedogenicRating, safeForPregnancy: data.safeForPregnancy,
            safeForSensitive: data.safeForSensitive, targetFocus: data.targetFocus || "",
          });
          
          if (data.goodForSkinTypes) {
            const types = data.goodForSkinTypes.split(",");
            setSkinTypes({
              Normal: types.includes("Normal"),
              Kering: types.includes("Kering"),
              Berminyak: types.includes("Berminyak"),
              Kombinasi: types.includes("Kombinasi"),
            });
          }
        } else {
        ...
        setMessage({ type: "error", text: "Gagal menarik data." });
      } finally {
        setIsFetching(false);
      }
    };
  ...
  if (isFetching) return <div className="p-12 text-center text-slate-500">Memuat data bahan...</div>;
    ...
    <div className="min-h-screen bg-slate-50 p-6 md:p-12">
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
          ...
          <p className="text-sm text-slate-500 mb-8 font-medium">Perbarui data bahan yang sudah ada.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                ...
                <input id="name" required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-black outline-none transition-all text-sm font-medium text-slate-900 bg-white" />
              </div>
              <div className="space-y-2">
                <label htmlFor="type" className="text-xs font-bold text-slate-700 uppercase">Kategori Logika</label>
                <select id="type" value={formData.type} onChange={(e) => setFormData({...formData, type: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-black outline-none transition-all text-sm font-medium text-slate-900 bg-white">
            ...
            <div className="space-y-2">
              ...
              <input id="aliases" type="text" value={formData.aliases} onChange={(e) => setFormData({...formData, aliases: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-black outline-none transition-all text-sm font-medium text-slate-900 bg-white" />
            </div>

            <div className="space-y-2">
              <label htmlFor="benefits" className="text-xs font-bold text-slate-700 uppercase">Manfaat Utama</label>
              <textarea id="benefits" required rows={3} value={formData.benefits} onChange={(e) => setFormData({...formData, benefits: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-black outline-none transition-all text-sm font-medium resize-none text-slate-900 bg-white" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <label htmlFor="comedogenic" className="text-xs font-bold text-slate-700 uppercase">Komedogenik (0-5)</label>
                <input id="comedogenic" type="number" min="0" max="5" value={formData.comedogenicRating} onChange={(e) => setFormData({...formData, comedogenicRating: parseInt(e.target.value)})} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-black outline-none text-sm font-medium text-slate-900 bg-white" />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {(Object.keys(skinTypes) as Array<keyof typeof skinTypes>).map((type) => (
                    <label key={type} className="flex items-center gap-2 cursor-pointer bg-slate-50 p-2 rounded-lg border border-slate-200 hover:bg-slate-100">
                      <input type="checkbox" checked={skinTypes[type]} onChange={() => handleSkinTypeChange(type)} className="w-4 h-4 accent-black" />
              ...
              <div className="space-y-2">
                <label htmlFor="focus" className="text-xs font-bold text-slate-700 uppercase">Fokus Utama</label>
                <select id="focus" value={formData.targetFocus} onChange={(e) => setFormData({...formData, targetFocus: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-black outline-none text-sm font-medium text-slate-900 bg-white">
            ...
            <button type="submit" disabled={isLoading} className="w-full py-4 bg-blue-600 text-white font-bold rounded-2xl hover:bg-blue-700 transition-all active:scale-95 disabled:bg-slate-300 shadow-md">
